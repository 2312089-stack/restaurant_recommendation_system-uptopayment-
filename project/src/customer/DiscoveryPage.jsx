...
const DiscoveryPage = ({ onBack, onShowDishDetails, onShowRestaurantMenu }) => {
  ...
  const fetchDishes = async (reset = false) => {
    try {
      ...
      if (sellerIds.length > 0) {
        try {
          const statusResponse = await fetch(`${API_BASE_URL}/seller-status/bulk`, {
            method: 'POST',
  ...
  const fetchRestaurants = async (reset = false) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/discovery/restaurants?${queryParams}`);
      const data = await response.json();
  ...
  const handleAddToCart = async (dish) => {
    ...
    // ✅ CHECK IF SELLER IS OFFLINE - Show notification instead of alert
...
const handleDishClick = (dish) => {
  ...
  console.log('🍽️ onShowDishDetails exists?:', typeof onShowDishDetails);
  console.log('🍽️ Calling onShowDishDetails with:', dishId);
  
  if (onShowDishDetails && typeof onShowDishDetails === 'function') {
    onShowDishDetails(dishId);
  } else {
    console.error('❌ onShowDishDetails is not a function!');
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {/* ✅ WISHLIST NOTIFICATION (Pink/Red) */}
      {wishlistMessage && (
