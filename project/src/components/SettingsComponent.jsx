...
const Settings = ({ onClose }) => {
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const response = await makeAuthenticatedRequest(buildApiUrl('/addresses'));
  ...
  const handleSaveAddress = async (addressData) => {
    try {
      const url = editingAddress 
        ? buildApiUrl(`/addresses/${editingAddress._id}`)
  ...
  const handleSetDefaultAddress = async (addressId) => {
    try {
      const response = await makeAuthenticatedRequest(
        buildApiUrl(`/addresses/${addressId}/set-default`),
        { method: 'PUT' }
      );
  ...
  const handleChangeEmail = async (newEmail, currentPassword) => {
    ...
    try {
      const response = await fetch(buildApiUrl('/settings-auth/change-email'), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleChangePassword = async (currentPassword, newPassword) => {
    ...
    try {
      const response = await fetch(buildApiUrl('/settings-auth/change-password'), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleResetPasswordRequest = async (email) => {
    ...
    setApiMessage({ type: '', text: '' });
    try {
      const response = await fetch(buildApiUrl('/settings-auth/reset-password-request'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
