...
export const CartProvider = ({ children }) => {
  ...
  const updateQuantity = async (dishId, quantity) => {
    ...
    try {
      ...
      const response = await makeAuthenticatedRequest(`${API_BASE_URL}/cart/item/${String(dishId)}`, {
        method: 'PUT',
  ...
  const getCartSummary = async () => {
    ...
    try {
      const response = await makeAuthenticatedRequest(`${API_BASE_URL}/cart/summary`);
      const data = await response.json();
