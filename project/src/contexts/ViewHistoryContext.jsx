...
import { API_BASE_URL } from "../config/api.js";const ViewHistoryContext = createContext();
...
export const ViewHistoryProvider = ({ children }) => {
  ...
  const trackView = useCallback(async (dish) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/view-history/track`, {
        method: 'POST',
        headers: {
  ...
  const loadRecentlyViewed = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/view-history/recently-viewed?limit=20`, {
        headers: {
  ...
  const loadMostViewed = useCallback(async (options = {}) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/view-history/most-viewed?${params}`);
      const data = await response.json();
  ...
  const clearRecentlyViewed = useCallback(async () => {
    ...
    if (token) {
      try {
        await fetch(`${API_BASE_URL}/view-history/recently-viewed`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const getDishStats = useCallback(async (dishId, days = 7) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/view-history/dish/${dishId}/stats?days=${days}`
      );
      const data = await response.json();
